...
class ds_weather extends Module implements WidgetInterface
{
    private $templateFile;

    public function __construct()
    {
        $this->name = 'ds_weather';
        $this->author = 'Damian';
        $this->version = '1.0.0';
        ...
        $this->displayName = $this->trans('DS Weather', array(), 'Modules.Comment.Admin');
        $this->description = $this->trans('Show me weather', array(), 'Modules.Comment.Admin');

        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:ds_weather/ds_weather.tpl';
    }
    ...
    {
        $this->context->controller->registerJavascript('modules-ps-manifest', 'modules/'.$this->name.'/app/dist/manifest.js', ['position' => 'bottom', 'priority' => 151]);
        $this->context->controller->registerJavascript('modules-ps-vendor', 'modules/'.$this->name.'/app/dist/vendor.js', ['position' => 'bottom', 'priority' => 151]);
    ...
    {
        return [
            'test' => $this->context->link->getModuleLink('ds_weather')
        ];
    }
